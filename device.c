...
uint8_t device_poll(USB_NKRO_Report_Data_t *report)
{
    ...
    // report->Keys[SC_TO_IDX(HID_KEYBOARD_SC__)] |= SC_TO_MSK(HID_KEYBOARD_SC__) * !(port_c & (1 << 4)); /* LAYER */
    ...
    // report->Keys[SC_TO_IDX(HID_KEYBOARD_SC__)] |= SC_TO_MSK(HID_KEYBOARD_SC__) * !(port_c & (1 << 5)); /* LAYER */
    /* port d */
